...
- Step 1, input equivalence classes:
  ...
  - `makeCopy()` has no parameters, but its behavior depends on the receiver state: a `Queen` with `color = BLACK`, `color = WHITE`, or `color = null`.
- Step 1, output equivalence classes:
  - The constructor creates a `Queen` whose type is fixed to `PieceType.QUEEN` and whose color is the supplied `color`.
  - `makeCopy()` returns a distinct `Queen` object whose type is `PieceType.QUEEN` and whose color matches the original queen, including `null`.
...
|             | System under test            | Expected output                                                                                       | Implemented? |
|-------------|------------------------------|-------------------------------------------------------------------------------------------------------|--------------|
| Test Case 4 | queen: `Queen(BLACK)`        | return a distinct `Piece`; copied piece is a `Queen`; copied piece has `type = QUEEN`; copied piece has `color = BLACK` | :x: |
| Test Case 5 | queen: `Queen(WHITE)`        | return a distinct `Piece`; copied piece is a `Queen`; copied piece has `type = QUEEN`; copied piece has `color = WHITE` | :x: |
| Test Case 6 | queen: `Queen(null)`         | return a distinct `Piece`; copied piece is a `Queen`; copied piece has `type = QUEEN`; copied piece has `color = null`  | :x: |
