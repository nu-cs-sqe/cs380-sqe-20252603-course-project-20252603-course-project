...
| Test Case 4 | design-target constructor arg: `color = WHITE` | pawn starts with `type = PAWN`, `color = WHITE`, and not-moved state for initial two-square movement | :x: |
...
| Test Case 12 | white pawn not moved; `from = Location(4, 6)`, `to = Location(4, 4)` | return `true`; two-square initial movement is legal from the white starting row when the pawn has not moved | :x: |
...
| Test Case 24 | white pawn; `from = Location(4, 6)`, `to = Location(5, 6)` | return `false`; horizontal movement with `dy = 0` is illegal | :x: |
| Test Case 25 | black pawn; `from = Location(4, 1)`, `to = Location(5, 1)` | return `false`; horizontal movement with `dy = 0` is illegal | :x: |
