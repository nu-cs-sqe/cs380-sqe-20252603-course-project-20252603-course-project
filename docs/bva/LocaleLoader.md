...
|-------------|-------------------|-----------------|--------------|
| Test Case 1 | `supported-locales.properties` has 2 valid entries | Map size is exactly 2, keys match parsed target locales, values match display keys | :x:          |
| Test Case 2 | `supported-locales.properties` is missing from classpath | Static initialization fails, throwing `IllegalStateException` | :x:          |
